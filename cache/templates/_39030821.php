		...
		<div class="row text-center mx-0 site-summarize-list">
            ...
            foreach ($data['summarize'] as $index => $val) {
                ...
                if(is_array($val)){
                    ...
                    $val['_last']  = $index == (count($data['summarize'])-1) ? true : false;
                    $val['sub']    = $sub;
                }
...
<div class="card mt-3">
	<div class="card-header">
		<h3 class="h6 mb-0 float-left"><?php echo $word['recom'];?></h3>
		<a href="<?php echo $c['market_url'];?>" target="_blank" class="float-right"><?php echo $word['columnmore'];?> <i class="fa fa-angle-right"></i></a>
	</div>
...
<div class="card mt-3">
	...
	<div class="card-body py-2 home-news-list" data-url="https://www.metinfo.cn/metv5news.php?fromurl=<?php echo $c['met_weburl'];?>&action=json&listnum=6">
		<?php
# MetInfo Enterprise Content Management System
# Copyright (C) MetInfo Co.,Ltd (http://www.metinfo.cn). All rights reserved.
defined('IN_MET') or exit('No permission');
