...
class index extends admin
{
    ...
    public function dosql_execute()
    {
        ...
        if (file_exists($filepath)) {
            ...
            if ($version != $old_version) {
                ...
                }elseif (version_compare($old_version, '7.0.0', '<')) {//7.0.0beta->7.0.0
                    //更新语言
                    $update_database->update_language($version);
                }
            }
    ...
    public function dogetLangData()
    {
        //return;
        global $_M;
        $sql = "select `name`,`value`,`site`,`no_order`,`array`,`app`,`lang` FROM {$_M['table']['language']} WHERE app = '' OR app = 0";
        ...
        file_put_contents(__DIR__ . "/v{$_M['config']['metcms_v']}lang_cn.json", json_encode($lang_cn, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
        file_put_contents(__DIR__ . "/v{$_M['config']['metcms_v']}lang_en.json", json_encode($lang_en, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));


        $lang_ini_cn_admin = "#\n";
        $lang_ini_cn_web = "#\n";
        foreach ($lang_cn as $row) {
            if ($row['site'] == 1) {
                $lang_ini_cn_admin .= "{$row['name']}={$row['value']}\n";
            }else{
                $lang_ini_cn_web .= "{$row['name']}={$row['value']}\n";
            }
        }

        $lang_ini_en_admin = "#\n";
        $lang_ini_en_web = "#\n";
        foreach ($lang_en as $row) {
            if ($row['site'] == 1) {
                $lang_ini_en_admin .= "{$row['name']}={$row['value']}\n";
            }else{
                $lang_ini_en_web .= "{$row['name']}={$row['value']}\n";
            }
        }

        file_put_contents(__DIR__ . "/language_cn1.ini", $lang_ini_cn_admin);
        file_put_contents(__DIR__ . "/language_cn.ini", $lang_ini_cn_web);
        file_put_contents(__DIR__ . "/language_en1.ini", $lang_ini_en_admin);
        file_put_contents(__DIR__ . "/language_en.ini", $lang_ini_en_web);
