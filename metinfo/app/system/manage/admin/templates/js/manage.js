...
(function(){
	...
	function contentShow(options){
		...
		options.module=='about' && view_type=='module' && $content_show_item.remove()/* && ($content_show_item=$content_show.find('.content-show-item[data-path="'+hash+'"]'))*/;
		if($content_show.find('.content-show-item[data-path="'+hash+'"]').length && $content_show_item.attr('data-loaded')){
			...
			metLoadTemp(url,options,$content_show_item,function(html){
				...
				loadFun();
				options.module=='about' && metui.use(['form','app_form'],function(){
                    setTimeout(function(){
