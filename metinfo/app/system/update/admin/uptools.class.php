...
class uptools extends admin
{
    ...
    public function docheck()
    {
        ...
        if ($metcms_v['value'] == $this->version && $_M['form']['to']!='to') {
            #echo "已是最新版本 不可再次升级<br><hr>";
            #return;
        }

        echo "<br>版本{$metcms_v['value']}<br><hr>";
        echo "<a href='{$_M['url']['own_form']}a=doupdata&action=updata_version'>1.更新系统版本号</a><br><hr>";
        echo "<a href='{$_M['url']['own_form']}a=doupdata&action=diff_fields'>2.数据表对比</a><br><hr>";
        echo "<a href='{$_M['url']['own_form']}a=doupdata&action=table_regist'>3.注册数据表</a><br><hr>";
        echo "<a href='{$_M['url']['own_form']}a=doupdata&action=add_config'>4.添加配置</a><br><hr>";
        echo "<a href='{$_M['url']['own_form']}a=doupdata&action=update_admin_column'>5.更新后台栏目</a><br><hr>";
        echo "<a href='{$_M['url']['own_form']}a=doupdata&action=update_language'>6.更新语言(修复后台语言丢失问题)</a><br><hr>";

        echo "<br><br><span>根据具体情况选择修复<span><hr>";
        ...
        if ($metcms_v['value'] == $this->version) {
            echo "<a href='{$_M['url']['own_form']}a=doupdata&action=job_repair'>修复招聘数据 (修复升级后添加招聘职位在和后台不显示的问题)</a><br><hr>";
            echo "<a href='{$_M['url']['own_form']}a=doupdata&action=update_admin_column'>修复无法编辑管理员的错误</a><br><hr>";
        }
        echo "<a href='{$_M['url']['own_form']}a=doupdata&action=template_repair'>修复默认模板配置不显示</a><br><hr>";
        echo "<a href='{$_M['url']['own_form']}a=doupdata&action=uiconfig_repair'>修复UI配置读取错误 (UI模板可是化显示错误，前台模板空白)</a><br><hr>";
    ...
    public function doupdata()
    {
        ...
        switch ($action) {
            case 'diff_fields'://数据表对比
                ...
                echo 'diff_fields complete';
                self::alter_table($version);
                echo 'alter_table complete';
                break;
            ...
            case 'template_repair'://修复默认模板配置不显示
                ...
                echo 'complete';
                break;
            case 'uiconfig_repair'://修复UI配置读取错误
                self::uiconfig_repair();
                echo 'complete';
                break;
        }
    }
    ...
    {
        global $_M;
        $diffs = self::get_diff_tables($version);
     ...
     * 更新数据表
     */
    /*public function _alter_table()
    {
        global $_M;
        $sql = "ALTER TABLE `{$_M['table']['job']}` 
        MODIFY COLUMN `class1` int(11) DEFAULT 0 ,
        MODIFY COLUMN `class2` int(11) DEFAULT 0 ,
        MODIFY COLUMN `class3` int(11) DEFAULT 0 ;";
        DB::query($sql);

        $sql = "ALTER TABLE `{$_M['table']['parameter']}` 
        MODIFY COLUMN `edit_ok` int(2) DEFAULT 1";
        DB::query($sql);
    }*/
    ...
    public function update_language($version)
    {
        ...
        foreach (array_keys($_M['langlist']['web']) as $lang) {
            ...
            if ($lang != 'en') {
                ...
                $path = __DIR__ . "/v{$version}lang_cn.json";
            }else{
                $json_sql = "https://www.metinfo.cn/upload/json/v{$version}lang_en.json";
                $path = __DIR__ . "/v{$version}lang_en.json";
            }
     ...
     * @return mixed
     */
    public function get_base_table($version = '')
    {
        $json_sql = "https://www.metinfo.cn/upload/json/v{$version}mysql.json";
        ##$path = PATH_WEB . "v{$version}mysql.json";
        $path = __DIR__ . "/v{$version}mysql.json";
     ...
     * @return array
     */
    public function get_diff_tables($version = '')
    {
        global $_M;
        $tables = self::list_tables();
        $base = self::get_base_table($version);
        if (!$base) {
            die('对照文件获取失败');
        }
     ...
     * 修复UI配置读取错误
     */
    public function uiconfig_repair()
    {
        global $_M;
        if ($_M['config']['tablepre'] != 'met_') {
            $per = $_M['config']['tablepre'];
            $sql = "SELECT * FROM {$_M['table']['ui_config']} WHERE ui_name LIKE '{$per}%'";
            $list = DB::get_all($sql);
            foreach ($list as $row) {
                $new_name = str_replace($per, 'met_', $row['ui_name']);
                $sql = "UPDATE {$_M['table']['ui_config']} SET ui_name = '{$new_name}' WHERE id = {$row['id']}";
                DB::query($sql);
            }
        }
        return;
