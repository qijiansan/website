   ...
   */
    ...
    public function checkFunction($site_url = '')
    {
        ...
        $items = array(
            ...
            array('unlink', 'danger', '支持', '函数未开启，无法清除缓存',  '函数'),
            array('opendir', 'danger', '支持', '无法列出目录下文件',  '函数'),
            array('scandir', 'danger', '支持', '无法列出目录下文件',  '函数'),
            array('curl_exec', 'danger', '支持', '系统无法远程获取内容，会导致有些操作不起作用或数据不显示',  '函数'),
            array('woff2', 'warning', '支持', '无法进行可视化编辑',  'woff'),
            array('PHP', 'danger', PHP_VERSION, 'pphp版本需要在5.3到7.2之间，否则无法安装使用程序', 'php'),
            array('openssl', 'warning', OPENSSL_VERSION_TEXT, OPENSSL_VERSION_TEXT.'模块未开启，无法发送邮件，且部分应用插件无法使用（如官方商城、一键导入微信文章等）', 'openssl'),
        );
