        ...
        if($my) {
           ...
           $this->update($this->new_ver);
            ...
            foreach ($_M['langlist']['admin'] as $key => $value) {
                ...
                $query = "INSERT INTO `{$_M['table']['config']}` (`name`, `value`, `mobile_value`, `columnid`, `flashid`, `lang`) VALUES
                ('met_website', '1', '0', '0','0','{$value['mark']}');";
            ...
            $time = time();
            $query = "INSERT INTO `{$_M['table']['applist']}` (`no`, `ver`, `m_name`, `m_class`, `m_action`, `appname`, `info`, `addtime`, `updatetime`) VALUES
            ({$this->appNo}, '{$this->new_ver}', 'website', 'website', 'doindex', '网站关闭', '让网站前台页面显示“网站正在建设中”或自定义文字、图片，网站还没有配置完成或网站维护期间可以用到该功能。', {$time}, {$time});";
            DB::query($query);  //应用注册

            //插入插件
            $query = "INSERT INTO `{$_M['table']['app_plugin']}` (`no_order`, `no`, `m_name`, `m_action`, `effect`) VALUES
